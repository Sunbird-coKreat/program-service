...
var chaiFetchMock = require('chai-fetch-mock')
var fetchMock = require('fetch-mock');
const expectedData = require("../testData/expectData")
chai.use(chaiFetchMock);

var mockClient = fetchMock;
...
describe('test', () => {
  ...
  it('calls fetch', () => {
    const id = "test"
    var url = `${envVariables.baseURL}/action/content/v3/hierarchy/${id}?mode=edit`;
    before(() => fetchMock.get(url,expectedData.successQuestionFromSection,{ name: 'all' }))
    // return fetch(url).then(() => {
    //   expect(fetchMock).route('all').to.have.been.called.with.args(['all', expectedData.successQuestionFromSection]);
    //   // console.log(expect(fetchMock).route('all').Assertion)
    // });
          getQuestionForSection(id,fetchMock)
            .then((response) => {
